...
function caesar_encrypt(plaintext, shift) {
    ...
    for (let char of plaintext) {
        if (char.match(/[a-z]/i)) {
            const shiftAmount = char.toUpperCase() === char ? 65 : 97;
            encrypted += String.fromCharCode((char.charCodeAt(0) - shiftAmount + shift) % 26 + shiftAmount);
        } else {
            encrypted += char;
        }
    }
...
function caesar_decrypt(ciphertext, shift) {
    ...
    for (let char of ciphertext) {
        if (char.match(/[a-z]/i)) {
            const shiftAmount = char.toUpperCase() === char ? 65 : 97;
            decrypted += String.fromCharCode((char.charCodeAt(0) - shiftAmount - shift + 26) % 26 + shiftAmount);
        } else {
            decrypted += char;
        }
    }
